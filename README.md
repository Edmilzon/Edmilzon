# Edmilzon Tudela

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&weight=500&size=24&pause=1000&color=6366F1&center=true&vCenter=true&width=480&lines=Backend+%26+DevOps+Engineer;Arquitecto+de+soluciones;NestJS+%C2%B7+Spring+Boot+%C2%B7+TypeScript;C%C3%B3digo+que+transforma+ideas" alt="Typing SVG" />
</p>

<p align="center">
  <a href="https://github.com/Edmilzon"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="mailto:edmilzon.luna@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://linkedin.com/in/edmilzon-tudela-5ab03b40b"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://twitter.com/EdmilzonLT"><img src="https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white" /></a>
  <a href="https://edmilzon-dev.vercel.app" target="_blank"><img src="https://img.shields.io/badge/Portfolio-6366F1?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
</p>

<p align="center">
  <b>📖 English version → <a href="README.en.md">README.en.md</a></b>
</p>

<hr />

## Sobre mí

Backend & DevOps Engineer de **Cochabamba, Bolivia**. Estudiante de **Ingeniería en Sistemas** en la **Universidad Mayor de San Simón (UMSS)**.

Transformo ideas en soluciones escalables. Mi stack fuerte: **NestJS**, **Spring Boot**, **TypeScript**, **PostgreSQL**, **Docker** y **AWS**. También desarrollo móvil con **Flutter** y frontend con **Next.js**.

Apasionado por la **arquitectura de software** (hexagonal, DDD, clean architecture, SOLID). Experto en **Linux** (Arch Linux) y **DevOps** con despliegues en AWS, Vercel, Railway y Render.

<hr />

## Ahora mismo

🚀 **Eco Bocado (FoodSave)** — APK en fase beta. **10 restaurantes registrados**, mapas por coordenadas y geolocalización. Conecta restaurantes con clientes para reducir el desperdicio de alimentos.

<hr />

## Proyectos Destacados

### Sistema Becas UMSS
> Plataforma de calificación para docentes. Carga de estudiantes por CSV, autenticación de docentes y administración de convocatorias.

**Stack:** Next.js · NestJS · PostgreSQL

**Impacto:** ✅ Producción real durante **2 gestiones (2024-2025)** · **100+ estudiantes** procesados · Uso simultáneo de docentes

[![Frontend](https://img.shields.io/badge/Frontend_🌐-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/frontend_PBI)

---

### Eco Bocado (FoodSave)
> App contra el desperdicio de alimentos. Conecta restaurantes con comida sobrante a clientes que la compran con descuento. Mapas con coordenadas reales.

**Stack:** Flutter · Dart · NestJS · PostgreSQL · TypeORM · JWT

**Impacto:** ✅ APK en beta · **10 restaurantes registrados** · Geolocalización integrada · Frontend + Backend desarrollados en solitario

[![Frontend (Flutter)](https://img.shields.io/badge/Frontend_📱-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/reduccion_desperdicio_alimentos)
[![Backend (NestJS)](https://img.shields.io/badge/Backend_⚙️-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/reduccion_desperdicio_backend)

---

### Plataforma Cursos
> Plataforma educativa con roles (estudiante, profesor, admin), gamificación, progreso y cursos de diversos temas.

**Stack:** Next.js · NestJS · PostgreSQL

**Impacto:** ✅ **20 cursos** publicados · **10 profesores** y **15 estudiantes** activos

[![Frontend](https://img.shields.io/badge/Frontend_🌐-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/plataforma_cursos_frontend)
[![Backend](https://img.shields.io/badge/Backend_⚙️-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/plataforma_cursos_backend)

---

### admi-ventas (Mermeladas Artesanales)
> App web de venta de mermeladas artesanales con autenticación, catálogo, carrito de compras y gestión de ventas con QR y efectivo.

**Stack:** Next.js · TypeScript · Tailwind CSS

**Impacto:** ✅ Ventas reales procesadas · **2 productos** (distintos tamaños) · Pagos QR y efectivo · Versión web + APK

[![Frontend](https://img.shields.io/badge/Frontend_🌐-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/admi-ventas-frontend)
[![Backend](https://img.shields.io/badge/Backend_⚙️-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/admi-ventas-backend)

---

### searchCar (Alquiler de Autos)
> Plataforma de alquiler de vehículos con búsqueda, reservas y múltiples servicios de calificación integrados.

**Stack:** Next.js · TypeScript

**Impacto:** ⏱️ **6 meses de desarrollo** — el proyecto más extenso. Arquitectura con múltiples servicios de calificación.

[![Frontend](https://img.shields.io/badge/Frontend_🌐-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/searchCar-frontend)
[![Backend](https://img.shields.io/badge/Backend_⚙️-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/searchCar-backend)

---

### EmotionDetect
> Chatbot que detecta emociones en texto usando K-means, TF-IDF y aprendizaje no supervisado.

**Stack:** Python · Scikit-learn · NLP

**Impacto:** ✅ 30 clusters emocionales · Chat en tiempo real · Registro automático de conversaciones

[![Repositorio](https://img.shields.io/badge/Repositorio-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Edmilzon/detecccion_emocion)

---

### Más proyectos

| Repositorio | Stack | Descripción |
|---|---|---|
| [user_blog_backend](https://github.com/Edmilzon/user_blog_backend) | Spring Boot + JPA + PostgreSQL | API REST con 4 entidades relacionadas |
| [oh_Sansi_backend](https://github.com/Edmilzon/oh_Sansi_backend) | PHP + Laravel | Gestor de olimpiadas y medallas |
| [Calculadora flujos efectivo](https://github.com/Edmilzon/Calculadora_flujos_efectivo_astro) | Astro + TypeScript | Gestión contable |
| [Tres en Raya IA](https://github.com/Edmilzon/Tres_en_Raya_Proyct_IA) | Python | IA con Minimax + BFS |
| [PetFinder](https://github.com/Edmilzon/search_web) | Python + SPARQL | Buscador semántico de mascotas |
| [proyect_mercado](https://github.com/Edmilzon/proyect_mercado_backend) | NestJS + PostgreSQL | E-commerce multi-usuario |
| [Plataforma Python](https://github.com/Edmilzon/plataforma_educatica_python) | Next.js + NestJS | Curso interactivo de Python |
| [loginRegister-backend](https://github.com/Edmilzon/loginRegister-backend) | Spring Boot | Auth con JWT |
| [loginRegister-backend-nestjs](https://github.com/Edmilzon/loginRegister-backend-nestjs) | NestJS | Auth con JWT |
| [pizarra_digital](https://github.com/Edmilzon/pizarra_digital) | APK Android | Pizarra para tablet |
| [Control_voz_gesto_java](https://github.com/Edmilzon/Control_voz_gesto_java) | Java + OpenCV | Control por voz y gestos |
| [Detectar_videos_imagen_java](https://github.com/Edmilzon/Detectar_videos_imagen_java) | Java + OpenCV | Visión por computadora |

<hr />

## Experiencia Freelance

| Proyecto | Rol | Stack | Duración |
|---|---|---|---|
| Sistema Becas UMSS (PBI) | Full Stack Developer | NestJS · Next.js · PostgreSQL | 1 año |
| Plataforma Cursos | Full Stack Developer | Next.js · NestJS · PostgreSQL | ~8 meses |
| searchCar (Alquiler Autos) | Full Stack Developer | Next.js · TypeScript | ~6 meses |
| Curso Aprendizaje Python | Developer / Instructor | Next.js · NestJS · PostgreSQL | ~6 meses |
| proyect_mercado (E-commerce) | Full Stack Developer | NestJS · TypeScript · PostgreSQL | ~5 meses |
| admi-ventas (Mermeladas) | Full Stack Developer | Next.js · TypeScript · Tailwind | ~4 meses |
| oh_Sansi (Olimpiadas) | Backend Developer | PHP · Laravel | ~3 meses |
| Eco Bocado (FoodSave) | Mobile + Backend | Flutter · NestJS · PostgreSQL | En desarrollo |

<hr />

## Estadísticas de GitHub

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Edmilzon&theme=tokyonight" alt="GitHub Profile Details" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Edmilzon&theme=tokyonight" alt="GitHub Stats" />
  <img src="https://streak-stats.demolab.com/?user=Edmilzon&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Edmilzon&theme=tokyonight" alt="Top Languages by Repo" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Edmilzon&theme=tokyonight" alt="Top Languages by Commit" />
</p>

<hr />

## Habilidades Técnicas

![Avanzado](https://img.shields.io/badge/Avanzado-22c55e?style=flat-square)
![Intermedio](https://img.shields.io/badge/Intermedio-eab308?style=flat-square)
![Básico](https://img.shields.io/badge/Básico-6366f1?style=flat-square)

| Nivel | Tecnologías |
|---|---|
| ![Avanzado](https://img.shields.io/badge/Avanzado-22c55e?style=flat-square) | TypeScript, JavaScript, Java (Spring Boot), Python, NestJS / Node.js, PostgreSQL, MongoDB, Linux (Arch Linux), Docker, AWS, Vercel, Git / GitHub, Arquitectura de Software (Hexagonal, DDD, Clean Architecture) |
| ![Intermedio](https://img.shields.io/badge/Intermedio-eab308?style=flat-square) | React / Next.js, React Native, Flutter (Dart), Vue.js, MySQL, PHP |
| ![Básico](https://img.shields.io/badge/Básico-6366f1?style=flat-square) | Kotlin, C#, Go, Rust, Kubernetes, Machine Learning |

<hr />

## Contacto

<p align="center">
  <a href="mailto:edmilzon.luna@gmail.com">
    <img src="https://img.shields.io/badge/edmilzon.luna@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://linkedin.com/in/edmilzon-tudela-5ab03b40b">
    <img src="https://img.shields.io/badge/in/edmilzon--tudela--5ab03b40b-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://twitter.com/EdmilzonLT">
    <img src="https://img.shields.io/badge/@EdmilzonLT-000000?style=for-the-badge&logo=x&logoColor=white" alt="X" />
  </a>
</p>

<p align="center">
  <a href="https://edmilzon-dev.vercel.app" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-edmilzon--dev.vercel.app-6366F1?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
  </a>
</p>

<hr />

<p align="center">
  <strong>Código que transforma ideas en soluciones</strong>
</p>
